  ...
  return (
    <header className="fixed shadow-md w-full h-16 px-2 md:px-4 z-50 bg-white">
      ...
      <div className="flex items-center h-full justify-between">
        <Link to={""}>
          <div className="h-20">
            <img src={"https://t3.ftcdn.net/jpg/01/84/09/00/360_F_184090069_ZhrjNMGzKPe6aNgrKTa6SOFhfTjkzjvO.jpg"} alt='QuickPick' className='h-16' />
          </div>
        </Link>

        <div className="flex item-center gap-4 md:gap-8">
          <nav className="flex gap-4 md:gap-7 text-base md:text-lg md:flex hidden">
            <Link to={""}>Home</Link>
            <Link to={"menu/64bbdc60e221178febdc3435"}>Products</Link>
            <Link to={"about"}>About</Link>
            <Link to={"contact"}>Contact</Link>
          </nav>
          <div className="relative text-2xl text-slate-700">
            <Link to={"cart"} >
            <HiShoppingCart />
            <div className="absolute -top-2 -right-2 text-white bg-blue-500 h-5 w-3 rounded-full m-0 p-0 text-sm text-center ">
          ...
          <div className="text-slate-600 cursor-pointer" onClick={handleShowMenu}>
            ...
            {showMenu && (
              <div className="absolute right-2 bg-white py-2 shadow drop-shadow-md flex flex-col min-w-[120px] text-center">
                ...
                {
                  userData.image ? <p className="cursor-pointer text-white px-2 bg-green-500" onClick={handleLogout}>Logout ({userData.firstName}) </p> : <Link to={'login'} className="whitespace-nowrap cursor-pointer text-white px-2 bg-blue-500">Login</Link>
                } 
